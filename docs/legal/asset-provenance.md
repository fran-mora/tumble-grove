...
| iOS app icon | `public/ios-app-icon.png`, generated on 25 September 2026 using the built-in image-generation tool with `public/fruits.png` as a style reference. Full prompt and technical conversion are recorded in `public/ios-app-icon-prompt.txt`. | The tool did not disclose a model identifier. Opaque 1024-pixel PNG for the asset catalogue; no platform emoji artwork is used in this icon. Generation does not establish copyrightability or non-infringement. |
...
| Round colour measurements | `app/fruit-colours.ts`, `app/fruit-selection.ts`, `docs/fruit-colours.md`. | Representative colours measured from existing project sprite pixels by `scripts/measure-fruit-colours.py`. No additional images or external datasets are bundled. |
