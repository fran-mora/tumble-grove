...
Use the moon/sun button in the top-right toolbar to switch dark mode on or off without restarting the round. The choice stays on this device, including offline launches; light mode is the default. The board, guides, fruit names, controls and dialogs share the chosen appearance.

Newly merged fruit show their names for about two seconds. Labels follow the result, stay upright, and remain readable with reduced motion enabled. Busy chain reactions show up to three recent names at a time. Choose **Inspect** above the arena to freeze the board and tap a fruit to see its name and highlight its silhouette; **Resume** returns to play. Inspection pauses gravity, merges and cooldowns and prevents pointer, keyboard and tool-driven drops. It becomes available after the first drop. Keyboard users can press **I** to inspect, use the arrows to select fruit, and press **Escape** to resume. Selected names and merge discoveries are also announced to screen readers. Neither feature reveals fruit that have not yet appeared.
...
Discovery tests cover all 110 rotated fruit silhouettes, overlapping hit targets, frozen physics and tilt during inspection, keyboard selection, reset/mode changes, merge label timing, reduced motion, and label bounds on a small canvas. Browser checks cover merge names, tap and keyboard inspection without accidental drops, resuming, Help preserving inspection, and 390 × 844, 320 × 568 and 568 × 320 layouts without page scrolling.
