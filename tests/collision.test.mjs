...
test('fruit flesh rests flush with the floor at any orientation',()=>{
  for(let kind=0;kind<11;kind++)for(const angle of [0,.4,1.8,3.1]){
    const game=new MergeGame();const body=game.addFruit(kind,220,HEIGHT);body.angle=angle;body.age=2;game.step();
    ...
    assert.ok(Math.abs(body.y+shape.maxY-(HEIGHT-.5))<.01);
  }
});
