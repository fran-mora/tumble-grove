  ...
  return <main className="app-shell">
    ...
    <section className="game-layout" aria-label="Tumble Grove game">
      ...
      <div className="play-column"><div className="play-controls"><div className={`gravity-control ${hud.mode==='gravity'?'is-on':''}`}>
        ...
        </div>{tiltBusy&&<button className="cancel-tilt" onClick={()=>tiltControllerRef.current?.abort()}>Cancel connection</button>}{tiltError&&<p className="tilt-error" role="alert">{tiltError}</p>}<div className="board-heading"><span>{hud.mode==='gravity'?'Tilt, drop, and roll together.':'Make room for something bigger.'}</span><button className="pause-button" disabled={hud.over||tiltBusy} onClick={()=>setPaused(p=>!p)} aria-label={paused?'Resume game':'Pause game'}>{paused?<Play size={13}/>:<Pause size={13}/>} {paused?'RESUME':'PAUSE'}</button></div></div>
        <div className="arena-stage"><div className={`game-board ${hud.danger?'in-danger':''} ${hud.mode==='gravity'?'gravity-arena':''}`} style={{'--arena-ratio':(WIDTH+2*VIEW_PADDING)/((hud.mode==='gravity'?WIDTH:HEIGHT)+2*VIEW_PADDING),aspectRatio:`${WIDTH+2*VIEW_PADDING}/${(hud.mode==='gravity'?WIDTH:HEIGHT)+2*VIEW_PADDING}`} as CSSProperties} >
          <canvas ref={canvasRef} className="game-canvas" width={WIDTH} height={hud.mode==='gravity'?WIDTH:HEIGHT} tabIndex={0} role="application" aria-label={`Tumble Grove play area. ${hud.mode==='gravity'?'Circular arena. Tilt your phone to change gravity. ':''}Current fruit: ${fruits[hud.current].name}. Use left and right arrows to aim, Space or Enter to drop, P to pause.`}
        ...
        </div></div><p className="control-hint"><MousePointer2 size={15}/><span className="desktop-hint">Move to aim · Click to drop</span><span className="touch-hint">{hud.mode==='gravity'?'Tilt to roll · Drag and release to drop':'Drag to aim · Release to drop'}</span><kbd>← → &nbsp; + &nbsp; Space</kbd></p></div>
