...
import { FRUITS, WIDTH, HEIGHT, STEP, MergeGame, type GameMode } from './engine';
import { loadSprites, renderGame } from './render';

type Modal = 'help' | 'restart' | 'win' | 'mode' | null;
const getHud = (g: MergeGame) => ({mode:g.mode,score:g.score,drops:g.drops,merges:g.merges,highest:g.highest,next:g.next,current:g.current,over:g.over,danger:g.danger>.1,watermelons:g.watermelons});
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    register({name:'read_fruit_merge_game',title:'Read Fruit Merge game',description:'Read score, fruit positions, current and next fruit, and whether a fruit can be dropped. The classic play area is 440 by 570; gravity mode uses a 440-diameter circle. Gravity and spawn coordinates are included. Fruit kinds are 0 through 10, cherry through watermelon.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>gameRef.current?.snapshot()});
  ...
  const aim=(clientX:number,clientY:number)=>{const rect=canvasRef.current?.getBoundingClientRect(),game=gameRef.current;if(rect&&rect.width&&game)game.setAimPoint((clientX-rect.left)/rect.width*WIDTH,(clientY-rect.top)/rect.height*game.height);};
  ...
  return <main className="app-shell">
    ...
    <section className="game-layout" aria-label="Fruit Merge game">
      ...
      <div className="play-column"><div className={`gravity-control ${hud.mode==='gravity'?'is-on':''}`}>
        ...
        <div className={`game-board ${hud.danger?'in-danger':''} ${hud.mode==='gravity'?'gravity-arena':''}`} >
