...
import { CircleHelp, VolumeX, Volume2, RotateCcw, ArrowRight, Trophy, MousePointer2, Pause, Play, Sparkles, Smartphone, Search, Palette, Moon, Sun } from 'lucide-react';
...
import { POWER_DETAILS, fruitPower } from './powers';
...
const getHud = (g: MergeGame) => {
  ...
  const moment=event?{id:event.id,chain:event.chain,points:event.points,cleared:event.cleared,name:g.getFruit(event.kind).name,power:event.power}:null;
  return {lineup:g.lineup,moment,powersEnabled:g.powersEnabled,choiceOptions:g.choiceOptions,mode:g.mode,score:g.score,drops:g.drops,merges:g.merges,highest:g.highest,next:g.next,current:g.current,over:g.over,danger:g.danger>.1,watermelons:g.watermelons,inspecting:g.inspecting,inspectedKind:g.bodies.find(b=>b.id===g.inspectedId)?.kind??null,hasFruit:g.bodies.length>0};
};
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const animate=(now:number)=>{
      ...
      if(!game.choiceOptions.length&&game.watermelons>previousWatermelons&&game.events.every(event=>game.time-event.time>=MERGE_HOLD_SECONDS+.65)){
  ...
  const aim=(clientX:number,clientY:number)=>{const point=boardPoint(clientX,clientY),game=gameRef.current;if(point&&game&&!game.paused&&!game.inspecting)game.setAimPoint(point.x,point.y);};
  const toggleInspect=()=>{const game=gameRef.current;if(!game||busyRef.current||modalRef.current)return;if(game.setInspecting(!game.inspecting)){pointRef.current=null;setPaused(false);sync();canvasRef.current?.focus();}};
  ...
  const changePowers=(enabled:boolean)=>{
    ...
    setNotice(enabled?'Fruit powers on. Matching fruit activate Gather, Zest or Choose.':'Fruit powers off. A fresh ordinary round.');sync();canvasRef.current?.focus();
  };
  ...
  const currentPower=POWER_DETAILS[fruitPower(hud.lineup[hud.current])];
  const closeDialog=()=>setModal(null);
  return <main className="app-shell">
    <header className="topbar"><a className="brand" href="./" aria-label="Tumble Grove home"><Fruit kind={0}/><span>tumble<span className="brand-light"> grove</span><span className="brand-dot">.</span></span></a><span className="mode-pill"><i/> {hud.mode==='gravity'?'GRAVITY MODE':'CLASSIC MODE'}{hud.powersEnabled?' · POWERS':''}</span><div className="toolbar"><button className="icon-button appearance-toggle" onClick={()=>setDarkMode(value=>!value)} aria-label="Dark mode" aria-pressed={darkMode} title={darkMode?'Switch to light mode':'Switch to dark mode'}>{darkMode?<Sun size={21}/>:<Moon size={21}/>}</button><button className="icon-button" onClick={toggleSound} aria-label={sound?'Mute sound':'Enable sound'} aria-pressed={sound} title={sound?'Mute sound':'Enable sound'}>{sound?<Volume2 size={21}/>:<VolumeX size={21}/>}</button><button className="icon-button" onClick={()=>setModal('help')} aria-label="How to play" title="How to play"><CircleHelp size={21}/></button></div></header>
    <section className="game-layout" aria-label="Tumble Grove game">
      <aside className="score-column"><div className="score-card"><span className="eyebrow">YOUR SCORE</span><strong className="score-number" aria-label={`Score: ${hud.score}`}>{hud.score.toLocaleString()}</strong><div className="best-score"><Trophy size={17}/><span>{hud.powersEnabled?'Best · powers':'Best'}</span><b>{best.toLocaleString()}</b></div></div><div className="next-card"><span className="eyebrow">UP NEXT</span><div className="next-fruit"><Fruit kind={hud.next}/></div><span title={fruits[hud.next].name}>{fruits[hud.next].name}</span></div><button className="new-game" disabled={tiltBusy} onClick={openRestart} aria-label="New game"><RotateCcw size={17}/><span>New game</span></button><p className="little-note">A little focus.<br/>A lot of fruit.</p></aside>
      <div className="play-column"><div className="play-controls"><div className="mode-controls"><div className={`gravity-control ${hud.mode==='gravity'?'is-on':''}`}>
        ...
        </div><div className={`gravity-control powers-control ${hud.powersEnabled?'is-on':''}`}><Sparkles size={22} aria-hidden="true"/><div className="gravity-label"><label htmlFor="powers-toggle">Fruit powers</label><span id="powers-description">{hud.powersEnabled?'Merge to Gather, Zest or Choose.':'A little magic in every match.'}</span></div><Switch id="powers-toggle" className="gravity-switch" checked={hud.powersEnabled} onCheckedChange={choosePowers} disabled={tiltBusy} aria-describedby="powers-description" aria-label="Fruit powers"/></div></div>{tiltBusy&&<button className="cancel-tilt" onClick={()=>tiltControllerRef.current?.abort()}>Cancel connection</button>}{tiltError&&<p className="tilt-error" role="alert">{tiltError}</p>}<div className="board-heading"><span>{hud.inspecting?'Take a closer look.':hud.powersEnabled?'Make a little magic.':hud.mode==='gravity'?'Tilt, drop, and roll together.':'Make room for something bigger.'}</span><div className="board-actions"><button className="pause-button inspect-button" disabled={hud.over||tiltBusy||!!hud.choiceOptions.length||!hud.hasFruit} onClick={toggleInspect} aria-label="Inspect fruit" aria-pressed={hud.inspecting} title="Pause and tap fruit to see their names"><Search size={14}/> INSPECT</button><button className="pause-button" disabled={hud.over||tiltBusy||!!hud.choiceOptions.length} onClick={togglePause} aria-label={paused||hud.inspecting?'Resume game':'Pause game'}>{paused||hud.inspecting?<Play size={13}/>:<Pause size={13}/>} {paused||hud.inspecting?'RESUME':'PAUSE'}</button></div></div></div>
        <p className={`round-caption ${hud.moment&&!hud.inspecting?'merge-moment':''}`} title={hud.moment&&!hud.inspecting?`${hud.moment.cleared?'Final pair cleared':hud.moment.name} · +${hud.moment.points} points`:hud.powersEnabled?`${currentPower.name}: ${currentPower.description}`:'A fresh mix from the whole collection, with a spread of fruit colours.'}>{hud.moment&&!hud.inspecting?<><Sparkles size={15} aria-hidden="true"/><strong key={hud.moment.id} className="merge-moment-copy">{hud.moment.cleared?'Final pair!':hud.moment.chain>1?`${hud.moment.chain}-step cascade!`:hud.moment.power?`${POWER_DETAILS[hud.moment.power].name}!`:'Beautifully grown.'}</strong><span className="merge-points">+{hud.moment.points}</span></>:hud.powersEnabled?<><Sparkles size={14} aria-hidden="true"/><strong>{currentPower.name}</strong><span className="power-caption-copy">{currentPower.description}</span></>:<><Palette size={14} aria-hidden="true"/><span className="round-caption-prefix">THIS ROUND</span><strong>A fresh mix</strong></>}</p>
        <div className="arena-stage"><div className={`game-board ${hud.inspecting?'is-inspecting':''} ${hud.danger?'in-danger':''} ${hud.mode==='gravity'?'gravity-arena':''}`} style={{'--arena-ratio':(WIDTH+2*VIEW_PADDING)/((hud.mode==='gravity'?WIDTH:HEIGHT)+2*VIEW_PADDING),aspectRatio:`${WIDTH+2*VIEW_PADDING}/${(hud.mode==='gravity'?WIDTH:HEIGHT)+2*VIEW_PADDING}`} as CSSProperties} >
          <canvas ref={canvasRef} className="game-canvas" width={WIDTH} height={hud.mode==='gravity'?WIDTH:HEIGHT} tabIndex={0} role="application" aria-describedby={hud.inspecting?'inspection-status':undefined} aria-label={hud.inspecting?'Tumble Grove inspection. Game paused. Tap a fruit to see its name, or use arrow keys to explore. Press Escape to resume.':`Tumble Grove play area. ${hud.mode==='gravity'?'Circular arena. Tilt your phone to change gravity. ':''}Current fruit: ${fruits[hud.current].name}.${hud.powersEnabled?` Power on merging: ${currentPower.name}. ${currentPower.description}`:''} Use left and right arrows to aim, Space or Enter to drop, P to pause, I to inspect fruit.`}
            onPointerDown={event=>{if(event.button!==0)return;event.currentTarget.focus();const game=gameRef.current;if(game?.inspecting){const point=boardPoint(event.clientX,event.clientY);if(point){game.inspectFruit(point.x,point.y);sync();}return;}if(game?.paused||game?.choiceOptions.length)return;pointRef.current=event.pointerId;event.currentTarget.setPointerCapture(event.pointerId);aim(event.clientX,event.clientY);unlockAudio();}}
            onPointerMove={event=>aim(event.clientX,event.clientY)}
            onPointerUp={event=>{if(gameRef.current?.inspecting||pointRef.current!==event.pointerId)return;aim(event.clientX,event.clientY);pointRef.current=null;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);doDrop();}}
          ...
          {!hud.drops&&!paused&&!hud.inspecting&&<div className="empty-board"><div className="empty-icon"><Fruit kind={0}/></div><h1>{hud.mode==='gravity'?'Give it a little tilt.':'Good things grow together.'}</h1><p>{hud.mode==='gravity'?<>Tilt to roll the fruit.<br/>Tap or drag to drop a new one.</>:<>Drop a fruit. Match a pair.<br/>See how big you can go.</>}</p><span className="empty-cta">{ready?'Click or tap to drop':'Getting the fruit ready…'} <ArrowRight size={16}/></span></div>}
        ...
        </div></div><p className={`control-hint ${hud.inspecting?'inspection-hint':''}`}>{hud.inspecting?<><Search size={15}/><span className="inspect-instruction">Paused · Tap a fruit to see its name</span></>:<><MousePointer2 size={15}/><span className="desktop-hint">Move to aim · Click to drop</span><span className="touch-hint">{hud.mode==='gravity'?'Tilt to roll · Drag and release to drop':'Drag to aim · Release to drop'}</span><kbd>← → &nbsp; + &nbsp; Space</kbd></>}</p><output id="inspection-status" className="sr-only" aria-live="polite" aria-atomic="true">{hud.inspecting?(hud.inspectedKind===null?'Game paused. Tap a fruit, or use the arrow keys, to learn its name.':`${fruits[hud.inspectedKind].name}. Level ${hud.inspectedKind+1}.${hud.powersEnabled?` Power: ${POWER_DETAILS[fruitPower(hud.lineup[hud.inspectedKind])].name}.`:''}`):''}</output></div>
      <aside className="goal-column"><span className="eyebrow">YOUR LATEST DISCOVERY</span><h2>Look what<br/>you grew.</h2><div className="watermelon-art"><Fruit kind={hud.highest}/></div><p>Match a pair.<br/>Discover what comes next.</p><div className="goal-note"><span>LEVEL {hud.highest+1}</span><strong>{fruits[hud.highest].name}</strong></div></aside>
    </section>
