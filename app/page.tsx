...
const getHud = (g: MergeGame) => {
  ...
  return {lineup:g.lineup,moment,mode:g.mode,score:g.score,drops:g.drops,merges:g.merges,highest:g.highest,next:g.next,current:g.current,over:g.over,danger:g.danger>.1,watermelons:g.watermelons,inspecting:g.inspecting,inspectedKind:g.bodies.find(b=>b.id===g.inspectedId)?.kind??null,hasFruit:g.bodies.length>0};
};
  ...
  return <main className="app-shell">
    ...
    <section className="game-layout" aria-label="Tumble Grove game">
      ...
      <div className="play-column"><div className="play-controls"><div className={`gravity-control ${hud.mode==='gravity'?'is-on':''}`}>
        ...
        <p className={`round-caption ${hud.moment&&!hud.inspecting?'merge-moment':''}`} title={hud.moment&&!hud.inspecting?`${hud.moment.cleared?'Final pair cleared':hud.moment.name} · +${hud.moment.points} points`:'A fresh mix from the whole collection, with a spread of fruit colours.'}>{hud.moment&&!hud.inspecting?<><Sparkles size={15} aria-hidden="true"/><strong key={hud.moment.id} className="merge-moment-copy">{hud.moment.cleared?'Final pair!':hud.moment.chain>1?`${hud.moment.chain}-step cascade!`:'Beautifully grown.'}</strong><span className="merge-points">+{hud.moment.points}</span></>:<><Palette size={14} aria-hidden="true"/><span className="round-caption-prefix">THIS ROUND</span><strong>A fresh mix</strong></>}</p>
