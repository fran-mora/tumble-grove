...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    register({name:'read_tumble_grove_game',title:'Read Tumble Grove game',description:'Read score, fruit positions, current and next fruit, and whether a fruit can be dropped. The classic play area is 440 by 570; gravity mode uses a 440-diameter circle. Gravity and spawn coordinates are included. Fruit levels are 0 through 10. Each round has a randomly selected theme. The snapshot lists this round’s selected character for each level.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>gameRef.current?.snapshot()});
