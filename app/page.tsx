...
import { CircleHelp, VolumeX, Volume2, RotateCcw, ArrowRight, Trophy, MousePointer2, Pause, Play, Sparkles, Smartphone, Search } from 'lucide-react';
...
const getHud = (g: MergeGame) => ({lineup:g.lineup,mode:g.mode,score:g.score,drops:g.drops,merges:g.merges,highest:g.highest,next:g.next,current:g.current,over:g.over,danger:g.danger>.1,watermelons:g.watermelons,inspecting:g.inspecting,inspectedKind:g.bodies.find(b=>b.id===g.inspectedId)?.kind??null,hasFruit:g.bodies.length>0});
...
export default function Home() {
  ...
  const reset=useCallback(()=>{pointRef.current=null;gameRef.current?.reset();setPaused(false);setModal(null);modalRef.current=null;setNotice('Fresh fruit, fresh start.');sync();canvasRef.current?.focus();},[sync]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    register({name:'read_tumble_grove_game',title:'Read Tumble Grove game',description:'Read score, fruit positions, current and next fruit, and whether a fruit can be dropped. The classic play area is 440 by 570; gravity mode uses a 440-diameter circle. Gravity and spawn coordinates are included. Fruit levels are 0 through 10. The snapshot lists this round’s selected character for each level.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>gameRef.current?.snapshot()});
