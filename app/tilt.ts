import { isNativeApp, requestNativeTilt, type NativeBridgeHost, type NativeEvent } from './native.ts';
export type GravityVector = {x:number;y:number};
type OrientationSample = {beta:number|null;gamma:number|null};
export type TiltHost = NativeBridgeHost & {
  isSecureContext:boolean;
  DeviceOrientationEvent?:{requestPermission?:()=>Promise<string>};
  screen?:{orientation?:{angle:number}};
  orientation?:number;
  addEventListener:(type:string,listener:(event:Partial<OrientationSample>&NativeEvent)=>void)=>void;
  removeEventListener:(type:string,listener:(event:Partial<OrientationSample>&NativeEvent)=>void)=>void;
};
...
export async function requestPhoneTilt(onSample:(gravity:GravityVector)=>void,options:{signal:AbortSignal;host?:TiltHost;timeoutMs?:number;onError?:(error:Error)=>void}):Promise<()=>void> {
  const {signal,timeoutMs=6500}=options;
  const host=options.host??window as unknown as TiltHost;
  if(signal.aborted)throw new DOMException('Cancelled','AbortError');
  if(isNativeApp(host))return requestNativeTilt(onSample,{signal,host,timeoutMs,onError:options.onError});
  ...
  return new Promise((resolve,reject)=>{
    ...
    const sample=(event:Partial<OrientationSample>)=>{
      if(stopped)return;
      const angle=host.screen?.orientation?.angle??host.orientation??0;
      const gravity=orientationGravity(event.beta??null,event.gamma??null,angle);
      if(!gravity)return;
      last={beta:event.beta??null,gamma:event.gamma??null};onSample(gravity);
